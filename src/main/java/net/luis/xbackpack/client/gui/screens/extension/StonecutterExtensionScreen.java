...
public class StonecutterExtensionScreen extends AbstractExtensionScreen {
	...
	protected void renderAdditional(@NotNull GuiGraphics graphics, float partialTicks, int mouseX, int mouseY, boolean open) {
		if (open) {
			...
			graphics.blit(RenderType::guiTextured, this.getTexture(), this.leftPos + this.imageWidth + 72, this.topPos + 143 + (int) (39.0 * this.scrollOffset), 95 + (this.isScrollBarActive() ? 0 : 12), 0, 12, 15, imageWidth, imageHeight);
	...
	private void renderButtons(@NotNull GuiGraphics graphics, int mouseX, int mouseY) {
		for (int index = this.startIndex; index < this.startIndex + 12 && index < this.recipes.size(); ++index) {
			...
			int imageWidth = this.extension.getImageWidth();
			int imageHeight = this.extension.getImageHeight();
			graphics.blit(RenderType::guiTextured, this.getTexture(), x, y - 1, 95, offset, 16, 18, imageWidth, imageHeight);
		}
	}
	...
	private void renderRecipes(@NotNull GuiGraphics graphics) {
		for (int index = this.startIndex; index < this.startIndex + 12 && index < this.recipes.size(); ++index) {
			...
			graphics.renderItem(this.recipes.get(index).value().resultDisplay().resolveForFirstStack(context), x, y);
		}
	}
		...
		if (open && this.shouldDisplayRecipes()) {
			...
			for (int index = this.startIndex; index < this.startIndex + 12 && index < this.recipes.size(); ++index) {
				...
				if (x >= 0.0 && y >= 0.0 && x < 16.0 && y < 18.0) {
					tooltipRenderer.accept(this.recipes.get(index).value().resultDisplay().resolveForFirstStack(context));
				}
			}
		}
	}
	...
	public void updateRecipes(boolean resetSelected) {
		this.recipes.clear();
		Objects.requireNonNull(this.minecraft.getConnection()).recipes().stonecutterRecipes().entries()
			.stream().map(SelectableRecipe.SingleInputEntry::recipe).map(SelectableRecipe::recipe).filter(Optional::isPresent).map(Optional::orElseThrow).forEach(this.recipes::add);
		if (resetSelected) {
