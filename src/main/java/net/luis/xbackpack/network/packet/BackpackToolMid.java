...
public class BackpackToolMid {
	...
	public void handle(Supplier<Context> context) {
		...
		context.get().enqueueWork(() -> {
			...
			ItemStack down = backpack.getStackInSlot(BackpackConstans.BACKPACK_TOOL_SLOT_MID).copy();
			if (BackpackConstans.VALID_TOOL_SLOT_ITEMS.contains(main.getItem())) {
				player.setItemInHand(InteractionHand.MAIN_HAND, down);
				backpack.setStackInSlot(BackpackConstans.BACKPACK_TOOL_SLOT_MID, main);
			}
		});
	}
